...
func addTools(server *mcp.Server, handlers *Handlers) {
	catalog := make(toolSchemaCatalog, 41)
	...
	addTypedTool(server, catalog, tool("object_put", "Store a local file or directory in object storage and return its object ID; the bridge reads the path, hashes it and uploads locally, so bytes never travel through the tool call. A directory is packed into one tar.gz first.", false, false, false), handlers.objectPut)
	addTypedTool(server, catalog, tool("object_get", "Download an object, or one of its versions, to a local path on the machine running this bridge.", false, true, false), handlers.objectGet)
	addTypedTool(server, catalog, tool("object_list", "List objects in a namespace, optionally including descendant namespaces.", true, true, false), handlers.objectList)
	addTypedTool(server, catalog, tool("object_meta", "Read one object's full record including its version list.", true, true, false), handlers.objectMeta)
	addTypedTool(server, catalog, tool("object_resolve", "Look up an object by namespace and name instead of by ID.", true, true, false), handlers.objectResolve)
	addTypedTool(server, catalog, tool("object_patch", "Change an object's name, tags, metadata, retained version count or expiry.", false, true, false), handlers.objectPatch)
	addTypedTool(server, catalog, tool("object_touch", "Recompute an object's expiry from now using a new retention.", false, true, false), handlers.objectTouch)
	addTypedTool(server, catalog, tool("object_delete", "Soft delete an object; it can be restored within 72 hours.", false, true, true), handlers.objectDelete)
	addTypedTool(server, catalog, tool("object_restore", "Restore a soft deleted object.", false, true, false), handlers.objectRestore)
